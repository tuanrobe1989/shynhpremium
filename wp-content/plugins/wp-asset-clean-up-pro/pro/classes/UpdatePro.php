...
class UpdatePro
{
	...
	public static function updateHandleMediaQueriesLoad()
	{
	    ...
	    if (! isset($mainVarToUse['wpacu_media_queries_load']['styles']) && ! isset($mainVarToUse['wpacu_media_queries_load']['scripts'])) {
			return;
		}
		...
		if (isset($mainVarToUse['wpacu_media_queries_load']['styles']) && ! empty($mainVarToUse['wpacu_media_queries_load']['styles'])) {
			foreach ($mainVarToUse['wpacu_media_queries_load']['styles'] as $styleHandle => $styleMediaQueryArray) {
		...
		if (isset($mainVarToUse['wpacu_media_queries_load']['scripts']) && ! empty($mainVarToUse['wpacu_media_queries_load']['scripts'])) {
			foreach ( $mainVarToUse['wpacu_media_queries_load']['scripts'] as $scriptHandle => $styleMediaQueryArray ) {
				$mediaQueryLoadEnable = isset( $styleMediaQueryArray['enable'] ) && $styleMediaQueryArray['enable'];
				$mediaQueryLoadValue  = isset( $styleMediaQueryArray['value'] ) ? stripslashes( trim($styleMediaQueryArray['value']) ) : '';
	...
	public static function updateAssetsPositions()
	{
		...
		foreach (array('styles', 'scripts') as $assetKey) {
			...
			if (isset($mainVarToUse[$postKey]) && ! empty($mainVarToUse[$postKey])) {
				foreach ($mainVarToUse[$postKey] as $handle => $position) {
